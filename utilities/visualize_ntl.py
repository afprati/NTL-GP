...
def visualize_ntl(data, test_x, test_y, test_g, model, model2, likelihood, T0, station_le, train_condition):
    ...
    for obs_id in []:
         ...
         plt.savefig("results/localnews_{}.png".format(obs_id))
         plt.close()
...
def visualize_localnews_MCMC(data, train_x, train_y, train_i, test_x, test_y, test_i, model,\
    ...
    expanded_test_D = torch.tensor(data.Treated.to_numpy()).double().unsqueeze(1).repeat(num_samples, 1, 1)
    ...
    result = pd.DataFrame({
         ...
         "Treated": expanded_test_D.reshape(-1)})
    ...
    for i in [0,1]:
         ...
         plt.title("Averaged " + LABEL + " Group Trends ")
         plt.axvline(x=T0, color='red', linewidth=0.5, linestyle="--")
         plt.savefig("results/ntl_MCMC_{}.png".format(LABEL))
         plt.close()
